...
export default function EditJobPage({ params }: { params: { id: string } }) {
  ...
  function onSubmit(data: JobFormValues) {
    ...
    updateDoc(jobRef, jobData)
        .then(() => {
            toast({
                title: "تم تحديث الوظيفة بنجاح!",
                description: `تم تحديث وظيفة "${data.title}".`,
            });
            router.push("/jobs");
        })
        .catch((serverError) => {
  ...
  const loading = userLoading || jobLoading;
  ...
  if (!job) {
    return (
        <div className="max-w-4xl mx-auto px-4 py-12 text-center">
            ...
            <p className="mt-2 text-muted-foreground">لم يتم العثور على الوظيفة التي تبحث عنها.</p>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <Card className="rounded-2xl custom-shadow">
        <CardHeader>
          <CardTitle className="text-2xl font-bold">تعديل الوظيفة: {job.title}</CardTitle>
        </CardHeader>
        <CardContent>
          <Form {...form}>
