...
export default function NewGoalPage() {
  ...
  function onSubmit(data: GoalFormValues) {
    ...
    addDoc(goalsCol, data)
      .then(() => {
        toast({
          title: "تمت إضافة الهدف بنجاح!",
          description: `تمت إضافة هدف جديد للمنصة.`,
        });
        router.push('/goals');
      })
      .catch((serverError) => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <Card className="rounded-2xl custom-shadow">
        <CardHeader>
          <CardTitle className="text-2xl font-bold">إضافة هدف جديد</CardTitle>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              />
              <Button type="submit" size="lg" className="font-bold rounded-lg" disabled={form.formState.isSubmitting}>إضافة الهدف</Button>
            </form>
          </Form>
        </CardContent>
      </Card>
