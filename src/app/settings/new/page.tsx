...
const settingFormSchema = z.object({
  ...
  value: z.string().url({ message: "الرجاء إدخال رابط صحيح." }),
});
...
export default function NewSettingPage() {
  ...
  function onSubmit(data: SettingFormValues) {
    ...
    addDoc(settingsCol, data)
      .then(() => {
        toast({
          title: "تمت إضافة الرابط بنجاح!",
          description: `تمت إضافة رابط '${data.name}' جديد للمنصة.`,
        });
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <Card className="rounded-2xl custom-shadow">
        <CardHeader>
          <CardTitle className="text-2xl font-bold">إضافة رابط تواصل جديد</CardTitle>
        </CardHeader>
        <CardContent>
          <Form {...form}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="مثال: تويتر" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
               ...
               <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="مثال: https://twitter.com/username" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
              <Button type="submit" size="lg" className="font-bold rounded-lg" disabled={form.formState.isSubmitting}>إضافة الرابط</Button>
            </form>
          </Form>
        </CardContent>
      </Card>
