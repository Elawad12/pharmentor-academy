...
export default function EditAccountPage({ params }: { params: { id: string } }) {
  ...
  const accountRef = useMemo(() => db ? doc(db, 'bankAccounts', params.id) : null, [db, params.id]);
  const { data: account, loading: accountLoading } = useDoc<BankAccount>(accountRef);
    ...
    updateDoc(accountRef, data)
      .then(() => {
        toast({
          ...
          description: `تم تحديث الحساب البنكي.`,
        });
  ...
  if (!account) {
    return (
        <div className="max-w-4xl mx-auto px-4 py-12 text-center">
            <h1 className="text-2xl font-bold">الحساب غير موجود</h1>
            <p className="mt-2 text-muted-foreground">لم يتم العثور على الحساب البنكي الذي تبحث عنه.</p>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-12">
      ...
      <Card className="rounded-2xl custom-shadow">
        <CardHeader>
          <CardTitle className="text-2xl font-bold">تعديل الحساب البنكي: {account.bankName}</CardTitle>
        </CardHeader>
        <CardContent>
          <Form {...form}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input placeholder="اسم صاحب الحساب..." {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
